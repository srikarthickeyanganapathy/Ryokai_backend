...
public class TaskAssignmentServiceImpl implements TaskAssignmentService {
    ...
    @Override
    @Transactional
    public TaskResponseDTO assignTask(com.example.taskflow.task.api.request.TaskAssignmentCommand cmd) {
        ...
        boolean isPersonal = cmd.getScope().getMode() == com.example.taskflow.task.domain.model.TaskMode.PERSONAL;
        ...
        // Crew scoping
        if (crewId != null) {
            com.example.taskflow.crew.domain.Crew crew = crewRepository.findById(crewId)
        ...
        if (assignee != null) {
            notificationService.createAndSend(
                assignee,
                creator,
                com.example.taskflow.notification.event.NotificationEvent.TASK_ASSIGNED,
    ...
    private TaskResponseDTO mapToTaskResponseDTO(Task task) {
        return new TaskResponseDTO(
            ...
            task.getDueDate(),
            task.getTags(),
            task.getChecklists() != null ? task.getChecklists().stream().map(item -> new com.example.taskflow.task.api.response.ChecklistItemDTO(item.getId(), item.getText(), item.getIsCompleted(), item.getDisplayOrder(), item.getCreatedBy() != null ? item.getCreatedBy().getUsername() : null)).collect(Collectors.toList()) : java.util.Collections.emptyList(),

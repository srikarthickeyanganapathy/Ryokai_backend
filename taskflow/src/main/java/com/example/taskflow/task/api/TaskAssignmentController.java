...
public class TaskAssignmentController {
    ...
    public ResponseEntity<TaskResponseDTO> createPersonalTask(@Valid @RequestBody TaskRequestDTO request,
        ...
        com.example.taskflow.task.api.request.TaskAssignmentCommand cmd = com.example.taskflow.task.api.request.TaskAssignmentCommand.builder()
                ...
                .assignor(creator)
                .assignee(assignee)
                .scope(com.example.taskflow.task.domain.model.TaskScope.personal())
                .build();
    ...
    @PreAuthorize("hasPermission(#request, 'TASK_CREATE')")
    public ResponseEntity<com.example.taskflow.task.api.response.BulkAssignResponseDTO> bulkAssignTasks(
            @Valid @RequestBody com.example.taskflow.task.api.request.BulkAssignRequestDTO request,
            @AuthenticationPrincipal UserDetails userDetails) {
        User creator = getCurrentUser(userDetails);
        String tagsJoined = request.getTags() != null ? String.join(",", request.getTags()) : null;
        com.example.taskflow.task.api.response.BulkAssignResponseDTO response = taskBulkAssignmentService.bulkAssignTasks(
                request.getTitle(),

...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    // Org tasks terminate at APPROVED, personal/crew at COMPLETED â€” must exclude both.
    @Query("SELECT CASE WHEN COUNT(t) > 0 THEN true ELSE false END FROM Task t " +
           "WHERE t.team.id = :teamId AND t.assignee.id = :assigneeId " +
           "AND t.currentStatus NOT IN (com.example.taskflow.task.domain.model.TaskStatus.COMPLETED, com.example.taskflow.task.domain.model.TaskStatus.APPROVED)")

...
import com.example.taskflow.organization.membership.dto.AdminLeaveRequestDTO;
...
public class OrganizationMembershipController {
    ...
    public ResponseEntity<MembershipResponseDTO> updateMemberRole(
            @PathVariable @Min(1) Long id,
            @PathVariable @Min(1) Long userId,
            @Valid @RequestBody com.example.taskflow.organization.rbac.dto.UpdateRoleRequestDTO request,
            @AuthenticationPrincipal UserDetails userDetails) {
        User currentUser = getCurrentUser(userDetails);
    ...
    public ResponseEntity<Void> leaveOrDissolveOrganization(
            @PathVariable @Min(1) Long id,
            @Valid @RequestBody com.example.taskflow.organization.membership.dto.AdminLeaveRequestDTO request,
            @AuthenticationPrincipal UserDetails userDetails) {
        User user = getCurrentUser(userDetails);

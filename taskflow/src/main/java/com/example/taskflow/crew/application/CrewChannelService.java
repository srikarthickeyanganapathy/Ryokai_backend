...
import com.example.taskflow.crew.dto.CrewChannelRequestDTO;
import com.example.taskflow.crew.dto.CrewMessageDTO;
...
import com.example.taskflow.crew.infrastructure.persistence.CrewMessageRepository;
...
public class CrewChannelService {
    ...
    public CrewMessageDTO editMessage(Long crewId, Long channelId, Long messageId, User user, CrewMessageRequestDTO dto) {
        ...
        if (!msg.getAuthor().getId().equals(user.getId())) {
            throw new com.example.taskflow.shared.exception.UnauthorizedActionException("Only the author can edit this message.");
        }
    ...
    public void deleteMessage(Long crewId, Long channelId, Long messageId, User user) {
        ...
        if (!isAuthor && !isCreator) {
            throw new com.example.taskflow.shared.exception.UnauthorizedActionException("Not authorized to delete this message.");
        }
    ...
    public TaskResponseDTO convertMessageToTask(Long crewId, Long channelId, Long messageId, User user, ConvertToTaskRequestDTO dto) {
        ...
        com.example.taskflow.task.api.request.TaskRequestDTO req = new com.example.taskflow.task.api.request.TaskRequestDTO();
        ...
        com.example.taskflow.task.api.request.TaskAssignmentCommand cmd = com.example.taskflow.task.api.request.TaskAssignmentCommand.builder()
                ...
                .assignor(user)
                .assignee(user)
                .scope(com.example.taskflow.task.domain.model.TaskScope.crew(crewId))
                .build();
    ...
    private void validateMembership(Long crewId, User user) {
        if (!crewMemberRepository.existsByIdCrewIdAndIdUserId(crewId, user.getId())) {
            throw new com.example.taskflow.shared.exception.UnauthorizedActionException("You are not a member of this crew.");
        }
    }
    ...
    private void validateCreator(Crew crew, User user) {
        if (!crew.getCreator().getId().equals(user.getId())) {
            throw new com.example.taskflow.shared.exception.UnauthorizedActionException("Only the crew creator can perform this action.");
        }
    }
}
